 ...
 * mounted components both call a hook like useTasks() at once with the same
 * base channel name, the second one to run tries to attach a
 * `postgres_changes` listener to a channel the first one already called
 * `.subscribe()` on — which throws. Giving every hook instance its own
 * channel avoids that collision.
 */
